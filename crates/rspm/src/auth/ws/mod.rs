...
mod tests {
    ...
    fn retirement_capacity_is_reserved_before_spawn_and_refuses_saturation() {
        ...
        let permit =
            try_reserve_retirement_slot(Arc::clone(&slots)).expect("first retirement slot");
        for _ in 0..2 {
            assert_eq!(
                try_reserve_retirement_slot(Arc::clone(&slots))
                    .expect_err("capacity must stay full"),
                AuthenticatedUserWsError::RetirementCapacityExhausted
            );
        }
        drop(permit);
        let recycled =
            try_reserve_retirement_slot(slots).expect("terminal release recycles capacity");
        drop(recycled);
    }
    ...
    async fn failed_retirement_transfer_is_retained_aborted_and_joined_without_panic() {
        ...
        let (retirement_state_tx, mut retirement_state_rx) =
            watch::channel(RetirementState::Active);
    ...
    async fn assert_cancelled_session_wait_transfers_to_process_custody(use_shutdown: bool) {
        ...
        let mut wait: core::pin::Pin<Box<dyn core::future::Future<Output = ()> + '_>> =
            if use_shutdown {
                Box::pin(async {
    ...
    fn full_official_trade_and_order_examples_retain_every_legacy_field() {
        ...
        let batch =
            AuthenticatedUserEventBatch::decode_frame(&body).expect("official fields decode");
    ...
    fn official_optional_fields_accept_absence_null_and_documented_empty_hash() {
        ...
        let batch =
            AuthenticatedUserEventBatch::decode_frame(&body).expect("nullable fields decode");
        let AuthenticatedUserEvent::Trade(trade) = &batch.as_slice()[0] else {
    ...
    fn zero_execution_economics_survive_authenticated_ws_decode() {
        ...
        let body = serde_json::to_vec(&fixture).expect("fixture encodes");
        let batch = AuthenticatedUserEventBatch::decode_frame(&body)
            .expect("retain raw zero private evidence");
        let AuthenticatedUserEvent::Trade(trade) = &batch.as_slice()[0] else {
            panic!("trade fixture decoded as order")
        };
    ...
    async fn heartbeat_loss_after_proof_revokes_caught_up_readiness() {
        ...
        let ws =
            AuthenticatedUserWs::connect_with_config(&endpoint, Credentials::default(), config)
    ...
    async fn poisoned_raw_frame_store_preserves_custody_and_rejects_ack() {
        ...
        let raw = AuthenticatedUserEventBatch::decode_frame(b"[]")
            ...
            .with_transport_context_for_test(
                frame_sequence,
                1,
                token.generation(),
                token.gap_version(),
