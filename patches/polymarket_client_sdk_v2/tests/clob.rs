...
mod unauthenticated {
    ...
    #[tokio::test]
    async fn missing_v2_fee_details_never_cache_authoritative_zero() -> anyhow::Result<()> {
        let server = MockServer::start();
        let client = Client::new(&server.base_url(), Config::default())?;
        let condition_id =
            b256!("4c27acaae6b9528e6121c226f0c7e253073c0ecdee87eed1bca5b2fe4028e6ee");
        ...
        let error = client
            .fee_info(token_id)
            .await
            .expect_err("missing fd must not become FeeInfo::default");
        assert!(error.to_string().contains("omitted fee details"));
...
mod authenticated {
    ...
    #[tokio::test]
    async fn post_order_initial_returns_without_trade_resolution_polling() -> anyhow::Result<()> {
        let server = MockServer::start();
        let client = create_authenticated(&server).await?;
        ...
        let order_mock = server.mock(|when, then| {
            ...
            then.status(StatusCode::OK).json_body(json!({
                ...
                "success": true,
                "takingAmount": "100",
                "tradeIDs": ["trade-awaiting-settlement"]
            }));
        });
        let trade_resolution_mock = server.mock(|when, then| {
            when.method(GET)
                .path("/data/trades")
                .query_param("id", "trade-awaiting-settlement");
            then.status(StatusCode::OK).json_body(trades_page(&[]));
        });
        ...
        let response = tokio::time::timeout(
            ...
            client.post_order_initial(signed_order),
        )
        .await
        .expect("initial POST acknowledgement must not wait on trade resolution")?;

        assert_eq!(response.order_id, "accepted-order-1");
        assert_eq!(response.trade_ids, vec!["trade-awaiting-settlement"]);
        assert!(response.transaction_hashes.is_empty());
        order_mock.assert();
        trade_resolution_mock.assert_hits(0);
