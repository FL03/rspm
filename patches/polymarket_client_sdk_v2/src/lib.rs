...
#[cfg_attr(
    ...
    tracing::instrument(
        ...
        fields(
            ...
            endpoint_class = tracing::field::Empty,
            status_code
        )
    )
)]
async fn request<Response: DeserializeOwned>(
    ...
    let path = error::redacted_request_path(request.url().path());

    #[cfg(feature = "tracing")]
    tracing::Span::current().record("endpoint_class", path.as_str());
    ...
    if !status_code.is_success() {
        ...
        tracing::warn!(
            ...
            method = %method,
            endpoint_class = %path,
            "API request failed; response details redacted"
        );
...
mod tests {
    ...
    #[cfg(feature = "tracing")]
    #[test]
    fn request_trace_and_status_hide_dynamic_path_query_and_body() {
        ...
        let hostile_body = "Bearer private-response-body";
        let dynamic_path = format!("/data/order/{hostile_order}");
        let request_mock = server.mock(|when, then| {
            when.method(GET).path(dynamic_path.as_str());
            then.status(400).body(hostile_body);
        });
        ...
        let layer = tracing_subscriber::fmt::layer()
            ...
            .with_writer(move || {
                ...
                CaptureWriter(Arc::clone(&captured_writer))
            });
        ...
        let result = tracing::subscriber::with_default(subscriber, || {
            runtime.block_on(async {
                ...
                let http_request = client
                    .get(format!(
                        "{}{dynamic_path}?owner={hostile_wallet}",
                        server.base_url()
                    ))
                    .build()
                    .expect("hostile-path request");
                request::<serde_json::Value>(&client, http_request, None).await
            })
        });
        request_mock.assert();
        let error = result.expect_err("hostile response must fail");
        let status = error
        ...
        let rendered = format!(
            "{error:?} {error} {}",
            captured.lock().expect("trace capture lock").join("")
        );
        assert!(rendered.contains("/data/order/{order_id}"));
        for secret in [hostile_order, hostile_wallet, hostile_body, "owner="] {
            assert!(!rendered.contains(secret), "request trace leaked {secret}");
        }
    }
