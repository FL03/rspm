...
///
/// Every returned subscription stream owns a keepalive for the client. The stream
/// therefore remains connected after all explicit [`Client`] handles are dropped.
/// Call [`Client::shutdown`] to terminate streams explicitly; otherwise dropping
/// the last stream releases the connection.
///
/// # Examples
///
/// ```rust, no_run
    ...
    pub fn new(endpoint: &str, config: Config) -> Result<Self> {
        ...
        Ok(Self {
            inner: Arc::new(ClientInner {
                ...
                channel_gate: StdMutex::new(()),
                shutdown_task: Mutex::new(()),
            }),
        })
    }
...
impl<S: State> Client<S> {
    /// Stop every initialized channel and wait for all WebSocket background tasks.
    ///
    /// Shutdown is terminal, idempotent, and shared by all clones. Existing
    /// subscription streams close and later subscription attempts fail.
    pub async fn shutdown(&self) {
        let _shutdown = self.inner.shutdown_task.lock().await;
        let resources = self.inner.take_channels_for_shutdown();
        for resource in &resources {
            resource.begin_shutdown();
        }
        for resource in resources {
    ...
    fn keepalive_stream<T, St>(&self, stream: St) -> impl Stream<Item = T> + use<T, St, S>
    where
        St: Stream<Item = T>,
    {
    ...
    pub fn subscribe_orderbook(
        ...
        let stream = resources.subscriptions.subscribe_market_with_interest(
            asset_ids,
        ...
        Ok(self.keepalive_stream(stream))
    }
    ...
    pub fn subscribe_last_trade_price(
        ...
        let stream = resources.subscriptions.subscribe_market_with_interest(
            asset_ids,
            MessageInterest::LAST_TRADE_PRICE,
            false,
        )?;
    ...
    pub fn subscribe_best_bid_ask(
        ...
        let stream = self
            ...
            .subscriptions
            .subscribe_market_with_interest(asset_ids, MessageInterest::BEST_BID_ASK, true)?;
    ...
    pub fn subscribe_new_markets(
        ...
        let stream = self
            ...
            .get_or_create_channel(ChannelType::Market)?
            .subscriptions
            .subscribe_market_with_interest(asset_ids, MessageInterest::NEW_MARKET, true)?;
    ...
    pub fn subscribe_market_resolutions(
        ...
        let stream = self
            ...
            .get_or_create_channel(ChannelType::Market)?
            .subscriptions
            .subscribe_market_with_interest(asset_ids, MessageInterest::MARKET_RESOLVED, true)?;
...
impl<S: State> ClientInner<S> {
    ...
    fn take_channels_for_shutdown(&self) -> Vec<ChannelResources> {
        let _gate = self
            .channel_gate
            .lock()
            .unwrap_or_else(PoisonError::into_inner);
        self.shutdown.store(true, Ordering::Release);
        let channels: Vec<_> = self.channels.iter().map(|entry| *entry.key()).collect();
        channels
            .into_iter()
            .filter_map(|channel| {
                self.channels
