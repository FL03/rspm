...
use crate::ws::connection::{ActivityGuard, ConnectionState};
...
pub struct SubscriptionManager {
    ...
    reconnection_cancellation: CancellationToken,
    reconnection_task: Mutex<Option<JoinHandle<()>>>,
}
...
impl SubscriptionManager {
    ...
    pub fn new(
        ...
        Self {
            ...
            reconnection_cancellation: CancellationToken::new(),
            reconnection_task: Mutex::new(None),
        }
    }
    ...
    pub fn start_reconnection_handler(self: &Arc<Self>) {
        ...
        let cancellation = self.reconnection_cancellation.clone();
        let handle = tokio::spawn(async move {
            let _task_guard = ActivityGuard::reconnection_task();
            let mut was_connected = state_rx.borrow().is_connected();

            loop {
                let changed = tokio::select! {
                    () = cancellation.cancelled() => break,
                    changed = state_rx.changed() => changed,
                };
                if changed.is_err() {
                    break;
                }
        ...
        match self.reconnection_task.try_lock() {
            Ok(mut task) if task.is_none() => *task = Some(handle),
            _ => handle.abort(),
        }
    }

    pub(crate) fn begin_shutdown(&self) {
        self.reconnection_cancellation.cancel();
    ...
    pub fn subscribe_market(
        ...
        self.subscribe_market_with_interest(asset_ids, MessageInterest::MARKET, false)
    }
    ...
    pub fn subscribe_market_with_options(
        ...
        self.subscribe_market_with_interest(asset_ids, MessageInterest::MARKET, custom_features)
    }

    /// Subscribe to public market data for one exact set of message types.
    ///
    /// Typed client APIs use this path so unrelated market traffic cannot occupy
    /// their bounded broadcast receiver. The public raw subscription methods
    /// deliberately retain broad [`MessageInterest::MARKET`] compatibility.
    pub(crate) fn subscribe_market_with_interest(
        &self,
        asset_ids: Vec<U256>,
            ...
            loop {
                match rx.recv().await {
                    ...
                    Err(RecvError::Lagged(n)) => {
                        ...
                        yield Err(WsError::Lagged { missed: n }.into());
                    }
