...
impl<OrderKind, K: AuthKind> OrderBuilder<OrderKind, K> {
    ...
    async fn build_payload(
        ...
        let version = self.client.resolve_version().await?;
...
impl<K: AuthKind> OrderBuilder<Limit, K> {
    ...
    ///
    /// Protocol-version mismatches are returned to the caller. The SDK never
    /// mutates its cached signing contract as a side effect of submission.
    ///
    /// # Errors
    ///
