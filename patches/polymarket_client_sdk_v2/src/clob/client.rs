...
impl<S: State> Client<S> {
    ...
    /// Resolves the CLOB server version once. Submission never mutates a
    /// successfully cached protocol contract; authority owners must rebuild
    /// a client after any mismatch.
    pub(crate) async fn resolve_version(&self) -> Result<u32> {
    ...
    /// Returns the V2 platform-fee exponent (`fd.e`) for `token_id`.
    ///
    /// # Errors
    ///
    ...
    pub async fn clob_market_info(&self, condition_id: &str) -> Result<ClobMarketInfoResponse> {
        ...
        let fee_info = response.fee_details.as_ref().map(|fd| FeeInfo {
            rate: fd.rate,
        ...
        for token in response.tokens.iter().flatten() {
            ...
            if let Some(fee_info) = fee_info {
                ...
                self.inner.fee_infos.remove(&token.token_id);
            }
            self.inner
                .token_condition_map
        ...
        self.inner
            ...
            .ok_or_else(|| {
                Error::validation(format!(
                    "V2 market metadata omitted fee details for token {token_id}"
                ))
            })
    }
...
impl<K: Kind> Client<Authenticated<K>> {
    ...
    /// Posts a signed order and returns the venue's initial response without
    /// performing any follow-up private reads.
    ///
    /// This is the submission primitive for callers that must classify the
    /// POST acknowledgement at the transport boundary. Settlement hashes can
    /// be enriched later from the returned trade IDs.
    pub async fn post_order_initial(&self, order: SignedOrder) -> Result<PostOrderResponse> {
        let request = self
            .client()
    ...
    /// Posts a signed order to the orderbook and best-effort enriches matched
    /// responses with settlement transaction hashes.
    ///
    ...
    pub async fn post_order(&self, order: SignedOrder) -> Result<PostOrderResponse> {
        ...
        let response = self.post_order_initial(order).await?;
        if defer_exec {
            return Ok(response);
        }
    ...
    pub async fn post_orders(&self, orders: Vec<SignedOrder>) -> Result<Vec<PostOrderResponse>> {
        ...
        let mut responses: Vec<PostOrderResponse> =
            crate::request(&self.inner.client, request, Some(headers)).await?;
    ...
    pub async fn delete_readonly_api_key(&self, key: &str) -> Result<()> {
        ...
        let path = crate::error::redacted_request_path(request.url().path());
        let headers = self.create_headers(&request).await?;
        ...
        if !status.is_success() {
            return Err(Error::status(status, method, path, "redacted"));
        }
